	...
	public boolean addEventInDB(Event event) {
		...
		try {
			...
			statement.executeUpdate(
					"INSERT INTO show_event (name_show, regie, distributie, data_show, number_of_tickets, sters) VALUES ('"
							+ event.getName() + "', '" + event.getRegie() + "', '" + event.getDistributie() + "', '"
	...
	public boolean updateEventInDB(Event event) {
		try {
			...
			statement.executeUpdate("UPDATE show_event SET regie = '" + event.getRegie() + "', distributie = '"
					+ event.getDistributie() + "', data_show = '" + event.getDataPremierei() + "', number_of_tickets = "
					+ event.getNumberOfTockets() + " WHERE name_show = '" + event.getName() + "';");
